...
// HandleRecordDownload handles POST /api/v1/plugins/{publisher}/{name}/{version}/download.
func (h *Handler) HandleRecordDownload(w http.ResponseWriter, r *http.Request) {
	publisher, name, version := extractDownloadPath(r.URL.Path)
...
// HandleGetStats handles GET /api/v1/plugins/{publisher}/{name}/stats.
func (h *Handler) HandleGetStats(w http.ResponseWriter, r *http.Request) {
	publisher, name := extractStatsPath(r.URL.Path)
	if publisher == "" || name == "" {
...
// extractDownloadPath parses /api/v1/plugins/{publisher}/{name}/{version}/download.
func extractDownloadPath(path string) (publisher, name, version string) {
...
// extractStatsPath parses /api/v1/plugins/{publisher}/{name}/stats.
func extractStatsPath(path string) (publisher, name string) {
	const prefix = "/api/v1/plugins/"
	path = strings.TrimPrefix(path, prefix)
